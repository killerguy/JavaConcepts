...
public class OldFashionedPersonValidator implements PersonValidator {
    ...
    public void validate(Person person) {
        ...
        if (person.getFirstName().length() < 2)
            throw new IllegalArgumentException("firstname : must have at least 2 characters");
        if (person.getFirstName().length() > 30)
            throw new IllegalArgumentException("firstname : must have less than 30 characters");

        if (person.getLastName() == null) throw new IllegalArgumentException("lastname : must not be null");
        if (person.getLastName().length() < 4)
            throw new IllegalArgumentException("lastname : must have at least 4 characters");
        if (person.getLastName().length() > 30)
            throw new IllegalArgumentException("lastname : must have less than 30 characters");

        if (person.getEmail() == null) throw new IllegalArgumentException("email : must not be null");
        if (person.getEmail().length() < 3)
            throw new IllegalArgumentException("email : must have at least 3 characters");
        if (person.getEmail().length() > 50)
            throw new IllegalArgumentException("email : must have less than 50 characters");
